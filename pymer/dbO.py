...
class dbO:
    ...
    def deliverDb(self,sessionNumber,numberOfRecords):
        retObj = []
        dbCurs = self.db.execute("""SELECT time,number,plusTwo,ao5 FROM TIMES WHERE session=? ORDER BY number DESC LIMIT ?;""",(sessionNumber, numberOfRecords))
    ...
    def renameSession(self,num,name):
        self.db.execute("UPDATE SESSIONS SET name = ? WHERE sessionNumber = ?",(name,num))
        self.db.commit()
    ...
    def removeRecord(self,sesh):
        sessionSoFar = self.deliverDb(sesh, 12)
        num = self.getNumber(sessionSoFar) - 1
        self.db.execute("DELETE FROM TIMES WHERE session=? AND number=?;",(sesh,num))
        self.db.commit()
